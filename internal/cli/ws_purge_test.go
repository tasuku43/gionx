...
func TestCLI_WS_Purge_ArchivedWorkspace_DeletesPathsAndCanCommitAndUpdatesDB(t *testing.T) {
...

	{
		...
		code := c.Run([]string{"ws", "purge", "--commit", "WS1"})
		if code != exitOK {
...
func TestCLI_WS_Purge_NoCommit_PurgesWithoutCreatingNewCommit(t *testing.T) {
	testutil.RequireCommand(t, "git")
...

	{
		...
		var err bytes.Buffer
		c := New(&out, &err)
		code := c.Run([]string{"ws", "purge", "--no-prompt", "--force", "--no-commit", "WS1"})
		if code != exitOK {
			t.Fatalf("ws purge --no-commit exit code = %d, want %d (stderr=%q)", code, exitOK, err.String())
		}
	}
	...
	if _, err := os.Stat(filepath.Join(env.Root, "workspaces", "WS1")); err == nil {
		t.Fatalf("workspaces/WS1 should not exist after purge --no-commit")
	}
	if _, err := os.Stat(filepath.Join(env.Root, "archive", "WS1")); err == nil {
		t.Fatalf("archive/WS1 should not exist after purge --no-commit")
	}
}
