...
func TestCLI_RepoDiscover_ExcludesExistingAndAddsSelected(t *testing.T) {
	...
	promptRepoDiscoverSelection = func(c *CLI, candidates []workspaceSelectorCandidate) ([]string, error) {
		return []string{"chatwork/newone"}, nil
	}
	defer func() {
...
func TestCLI_RepoRemove_RemovesSelectedRegisteredRepo(t *testing.T) {
	...
	env := testutil.NewEnv(t)
	env.EnsureRootLayout(t)
	repoSpec1 := prepareRemoteRepoSpecWithName(t, runGit, "github.com", "chatwork", "remove-target")
	repoSpec2 := prepareRemoteRepoSpecWithName(t, runGit, "github.com", "chatwork", "keep-target")
	...
	promptRepoRemoveSelection = func(c *CLI, candidates []workspaceSelectorCandidate) ([]string, error) {
		return []string{"chatwork/remove-target"}, nil
	}
	...
	var count int
	if err := db.QueryRowContext(context.Background(), "SELECT COUNT(1) FROM repos WHERE repo_key = ?", "chatwork/remove-target").Scan(&count); err != nil {
		t.Fatalf("query removed repo count: %v", err)
	}
	if count != 0 {
		t.Fatalf("removed repo still exists: count=%d", count)
	}
	if err := db.QueryRowContext(context.Background(), "SELECT COUNT(1) FROM repos WHERE repo_key = ?", "chatwork/keep-target").Scan(&count); err != nil {
...
func TestCLI_RepoRemove_FailsWhenRepoBoundToWorkspace(t *testing.T) {
	...
	env := testutil.NewEnv(t)
	env.EnsureRootLayout(t)
	repoSpec := prepareRemoteRepoSpecWithName(t, runGit, "github.com", "chatwork", "bound-repo")
	...
	var repoUID string
	if err := db.QueryRowContext(context.Background(), "SELECT repo_uid FROM repos WHERE repo_key = ?", "chatwork/bound-repo").Scan(&repoUID); err != nil {
	...
	var err bytes.Buffer
	c := New(&out, &err)
	code := c.Run([]string{"repo", "remove", "chatwork/bound-repo"})
	if code != exitError {
...
func TestCLI_RepoGC_RemovesOrphanBareRepo(t *testing.T) {
	...
	env := testutil.NewEnv(t)
	env.EnsureRootLayout(t)
	repoSpec := prepareRemoteRepoSpecWithName(t, runGit, "github.com", "chatwork", "gc-target")
	...
	}
	{
		...
		var err bytes.Buffer
		c := New(&out, &err)
		if code := c.Run([]string{"repo", "remove", "chatwork/gc-target"}); code != exitOK {
			t.Fatalf("repo remove exit code = %d, want %d (stderr=%q)", code, exitOK, err.String())
		}
	}
	...
	c := New(&out, &err)
	c.In = stdinR
	code := c.Run([]string{"repo", "gc", "chatwork/gc-target"})
	if code != exitOK {
...
func TestCLI_RepoGC_FailsWhenNoEligibleCandidates(t *testing.T) {
	...
	env := testutil.NewEnv(t)
	env.EnsureRootLayout(t)
	repoSpec := prepareRemoteRepoSpecWithName(t, runGit, "github.com", "chatwork", "gc-blocked")
	...
	var err bytes.Buffer
	c := New(&out, &err)
	code := c.Run([]string{"repo", "gc", "chatwork/gc-blocked"})
	if code != exitError {
