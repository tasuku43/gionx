...
func TestCLI_WS_Reopen_NoCommit_ReopensWithoutCreatingNewCommit(t *testing.T) {
	testutil.RequireCommand(t, "git")
...

	{
		...
		var err bytes.Buffer
		c := New(&out, &err)
		code := c.Run([]string{"ws", "reopen", "--no-commit", "WS1"})
		if code != exitOK {
			t.Fatalf("ws reopen --no-commit exit code = %d, want %d (stderr=%q)", code, exitOK, err.String())
		}
	}
	...
	if _, err := os.Stat(filepath.Join(env.Root, "archive", "WS1")); err == nil {
		t.Fatalf("archive/WS1 should not exist after reopen --no-commit")
	}
	if _, err := os.Stat(filepath.Join(env.Root, "workspaces", "WS1")); err != nil {
		t.Fatalf("workspaces/WS1 should exist after reopen --no-commit: %v", err)
	}
}
